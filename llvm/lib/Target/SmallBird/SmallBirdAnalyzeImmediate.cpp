//===-- SmallBirdAnalyzeImmediate.cpp - Analyze Immediates ---------------------===//
//
...
SmallBirdAnalyzeImmediate::Inst::Inst(unsigned O, unsigned I) : Opc(O), ImmOperand(I) {}

// Add I to the instruction sequences.
void SmallBirdAnalyzeImmediate::AddInstr(InstSeqLs &SeqLs, const Inst &I) {
...
void SmallBirdAnalyzeImmediate::GetInstSeqLsADDiu(uint64_t Imm, unsigned RemSize,
                                             InstSeqLs &SeqLs) {
  GetInstSeqLs((Imm + 0x8000ULL) & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ADDiu, Imm & 0xffffULL));
}

void SmallBirdAnalyzeImmediate::GetInstSeqLsORi(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
  GetInstSeqLs(Imm & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ORi, Imm & 0xffffULL));
}

void SmallBirdAnalyzeImmediate::GetInstSeqLsSHL(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
...
void SmallBirdAnalyzeImmediate::GetInstSeqLs(uint64_t Imm, unsigned RemSize,
                                        InstSeqLs &SeqLs) {
...
// are replaced with
//  LUi 0x444
void SmallBirdAnalyzeImmediate::ReplaceADDiuSHLWithLUi(InstSeq &Seq) {
...
void SmallBirdAnalyzeImmediate::GetShortestSeq(InstSeqLs &SeqLs, InstSeq &Insts) {
...
const SmallBirdAnalyzeImmediate::InstSeq
&SmallBirdAnalyzeImmediate::Analyze(uint64_t Imm, unsigned Size,
                               bool LastInstrIsADDiu) {
  this->Size = Size;

  ADDiu = SmallBird::ADDiu;
  ORi = SmallBird::ORi;
  SHL = SmallBird::SHL;
  LUi = SmallBird::LUi;
